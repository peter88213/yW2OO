...
# If CREATE_BOOKMARKS is True, sceti adds html anchors to be
# auto-converted into Open/LibreOffice bookmarks.
CREATE_BOOKMARKS = False

# SCENE_LIST is a csv file to be imported by Open/Libreoffice Calc.
# It contains hyperlinks to the bookmarks generated by sceti.
SCENE_LIST = 'Exported Project.csv'
...
def create_scene_list(bookmarks, sceneTitles):
    """ Generate a csv scene list. """
    odtPath = (os.getcwd().replace('\\', '/') + '/' +
               HTML_FILE).replace(' ', '%20').replace('html', 'odt')
    if CREATE_BOOKMARKS:
        csvData = ['Scene link,Scene title\n']
    else:
        csvData = ['Scene title\n']
    sceneTotal = len(sceneTitles)
    sceneCount = 0
    while sceneCount < sceneTotal:
        if CREATE_BOOKMARKS:
            csvData.append('=HYPERLINK("file:///'
                           + odtPath + '#'
                           + bookmarks[sceneCount] + '";"'
                           + bookmarks[sceneCount] + '"),"'
                           + sceneTitles[sceneCount].rstrip() + '"\n')
        else:
            csvData.append('"' + sceneTitles[sceneCount].rstrip() + '"\n')
        sceneCount = sceneCount + 1

    with open(SCENE_LIST, 'w') as f:
        try:
            f.writelines(csvData)
            print('Hyperlink list "' + SCENE_LIST + '" created.\n')
        except:
            print('ERROR: Cannot write "' + SCENE_LIST + '".\n')
...
def insert_scene_titles(bookTitle, htmlInput, sceneTitles):
    ...
    try:
        ...
        sys.exit(1)
    create_scene_list(bookmarks, sceneTitles)
